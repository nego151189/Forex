...
// ✅ Agrega una nueva condición visual
function agregarCondicionDesdeFormulario() {
  const container = document.getElementById("condicionesContainer");
  if (!container) return;
  const grupo = document.createElement("div");
  grupo.className = "grupo-condiciones mb-3";

  const fila = document.createElement("div");
  fila.className = "fila-condicion row g-2 align-items-center mb-2";

  fila.innerHTML = `
    <div class="col-md-3">
      <select class="form-select campo" required>
    ...
    <div class="col-md-3">
      <select class="form-select operador" required>
    ...
    <div class="col-md-4">
      <input type="text" class="form-control valor" placeholder="Valor a comparar" required />
    </div>
    <div class="col-md-2">
      <button type="button" class="btn btn-danger btn-sm eliminar-condicion"><i class="fas fa-trash-alt"></i></button>
    </div>
  `;

  fila.querySelector(".eliminar-condicion").addEventListener("click", () => {
    fila.remove();
  });
...
// ✅ Guarda la parametrización completa
function guardarParametrizacion() {
  const condiciones = [];

  document.querySelectorAll('.grupo-condiciones').forEach(grupo => {
    grupo.querySelectorAll('.fila-condicion').forEach(fila => {
      const campo = fila.querySelector('.campo')?.value;
      const operador = fila.querySelector('.operador')?.value;
      const valor = fila.querySelector('.valor')?.value;

      if (campo && operador && valor) {
        condiciones.push({ campo, operador, valor });
      } else {
        console.warn("Condición inválida ignorada:", { campo, operador, valor });
      }
    });
  });

  const resultado = document.getElementById('resultadoEsperado')?.value;

  if (!resultado || condiciones.length === 0) {
    alert('Debes agregar al menos una condición válida y un resultado.');
    return;
  }
  ...
  console.log("📦 Guardando producto:", doc);

  firebase.firestore().collection("productosFinancieros").add(doc)
    .then(() => {
      alert("✅ Producto guardado correctamente.");
      document.getElementById('resultadoEsperado').value = "";
      document.getElementById('condicionesContainer').innerHTML = "";
      agregarCondicionDesdeFormulario();
      cargarProductos();
    })
    .catch((error) => {
...
// ✅ Muestra todos los productos ya parametrizados
function cargarProductos() {
  firebase.firestore().collection("productosFinancieros")
    ...
    .then(snapshot => {
      ...
      if (snapshot.empty) {
        contenedor.innerHTML = '<p class="text-muted">No hay productos registrados.</p>';
        return;
      }
      ...
      snapshot.forEach(doc => {
        ...
        div.className = "producto-registrado card mb-3 p-3";

        const fecha = new Date(data.creadoEn).toLocaleString();

        div.innerHTML = `
          <div class="row">
            <div class="col-md-8">
              <h5><i class="fas fa-clipboard-check me-2"></i>Resultado: ${data.resultado}</h5>
              <ul class="mb-2">
                ${(data.condiciones || []).map(c =>
                  `<li><strong>${c.campo}</strong> ${c.operador} ${c.valor}</li>`
                ).join("")}
              </ul>
              <small class="text-muted">Registrado por ${data.creadoPor || '-'} el ${fecha}</small>
            </div>
            <div class="col-md-4 text-end">
              <button class="btn btn-sm btn-danger eliminar-producto" data-id="${doc.id}">
        ...
        div.querySelector(".eliminar-producto").addEventListener("click", () => {
          if (confirm("¿Estás seguro de eliminar este producto?")) {
            firebase.firestore().collection("productosFinancieros").doc(doc.id).delete()
              .then(() => {
...
// Función para agregar condición desde el formulario
function agregarCondicionDesdeFormulario() {
    ...
    if (!campo || !operador || !valor) {
        alert("Debes llenar todos los campos para agregar una condición.");
        return;
    }
    ...
    li.innerHTML = `
        <div>
            <strong>${campo}</strong> ${operador} <span class="text-primary">${valor}</span>
            <input type="hidden" class="campo-condicion" value="${campo}">
            <input type="hidden" class="operador-condicion" value="${operador}">
            <input type="hidden" class="valor-condicion" value="${valor}">
        </div>
...
// Asociar evento al botón (por si falla el onclick directo)
document.addEventListener("DOMContentLoaded", function () {
    const botonAgregar = document.getElementById("agregarCondicion");
    if (botonAgregar) {
        botonAgregar.addEventListener("click", agregarCondicionDesdeFormulario);
    } else {
        console.error("❌ No se encontró el botón con ID 'agregarCondicion'");
    }
});

// ✅ Función para renderizar visualmente un producto como tarjeta
function renderProducto(param) {
  const contenedor = document.getElementById('productosRegistrados');
  if (!contenedor) return;
  ...
  div.innerHTML = `
    <div class="condiciones">${(param.condiciones || []).join('<br>')}</div>
    <div class="resultado">Resultado: ${param.resultado}</div>
    <div class="fecha">${param.fecha || 'Sin fecha'}</div>
    <div class="acciones">
      <button class="btn btn-sm btn-outline-danger" onclick="eliminarProducto('${param.id}')">
        <i class="fas fa-trash"></i>
      </button>
    </div>
  `;
