...
// En main.js - MODIFICAR initializeTradingSystem:
    ...
    // ✅ INICIALIZAR SISTEMA DE TRADING CON PERSISTENCIA
...
updateEvolutionMetrics(stats) {
    ...
    async generateAdvancedSignal() {
        ...
        try {
            ...
            if (signal.confidence > 0.65 && signal.riskReward >= 1.8 && signal.action !== 'HOLD') {
...
async loadRealData() {
    ...
    // ✅ CONFIGURAR EVENT LISTENERS    // ✅ CONFIGURAR EVENT LISTENERS
    setupEventListeners() {
        ...
        // ✅ === NUEVO CÓDIGO AGREGADO ===
        // Botón de guardado manual si existe
        const saveModelBtn = document.getElementById('saveModelBtn');
        if (saveModelBtn) {
            saveModelBtn.addEventListener('click', () => {
                this.saveModelManually();
            });
        }
        // === FIN NUEVO CÓDIGO ===
    }
    ...
    // ✅ ENTRENAR MODELO ML
// EN main.js - REEMPLAZAR la función trainModel:
...
async trainModel() {
    ...
    if (trainButton) {
        ...
        try {
            ...
            // ✅ CORRECCIÓN CRÍTICA: Usar el modelo del trading system que SÍ tiene persistencia
            let mlModelToTrain;
            
            if (this.tradingSystem && this.tradingSystem.mlModel) {
                console.log('🔧 Usando modelo del trading system con persistencia configurada');
                mlModelToTrain = this.tradingSystem.mlModel;
            } else {
                console.log('⚠️ Usando modelo local (sin persistencia)');
                mlModelToTrain = this.mlModel;
                
                // Configurar persistencia temporal
                if (!mlModelToTrain.persistence && this.currentInstrument) {
                    mlModelToTrain.persistence = new MLPersistenceSystem();
                    mlModelToTrain.instrument = this.currentInstrument;
                }
            }
            
            // ✅ VERIFICAR ANTES DE ENTRENAR
            console.log('🔍 Estado persistencia antes de entrenar:', {
                hasPersistence: !!mlModelToTrain.persistence,
                hasInstrument: !!mlModelToTrain.instrument,
                instrument: mlModelToTrain.instrument
            });
            
            const accuracy = await mlModelToTrain.trainWithBacktest(this.currentData);
    ...
    cleanup() {
        ...
        // ✅ NUEVA FUNCIÓN AGREGADA - Guardado manual de modelo
    async saveModelManually() {
        if (!this.tradingSystem || !this.tradingSystem.mlModel) {
            this.showNotification('❌ No hay modelo para guardar', 'error');
            return;
        }

        try {
            const saved = await this.tradingSystem.mlModel.saveToStorage();
            if (saved) {
                this.showNotification('💾 Modelo guardado exitosamente', 'success');
                
                // Mostrar estadísticas del modelo guardado
                const stats = this.tradingSystem.mlModel.getEvolutionStats();
                console.log('📊 Estadísticas del modelo guardado:', stats);
            } else {
                this.showNotification('❌ Error guardando modelo', 'error');
            }
        } catch (error) {
...
// En main.js - buscar función loadInstrumentData
async function loadInstrumentData(instrument) {
    try {
        console.log(`📊 Cargando datos REALES para ${instrument}...`);
        
        // ✅ FORZAR 5000 VELAS SIEMPRE
        const data = await getTwelveDataReal(instrument, '15min', 5000);
        
        if (!data || data.length === 0) {
            throw new Error('No se pudieron cargar datos reales');
        }
        
        console.log(`✅ Datos REALES cargados: ${data.length} velas`);
        return data;
    } catch (error) {
        console.error('❌ Error cargando datos reales:', error);
        return loadSimulatedData(instrument);
    }
}
...
// ✅ FUNCIÓN DE VERIFICACIÓN DE ESTADO DEL MODELO
function checkModelStatus() {
    if (!app || !app.tradingSystem) {
        alert('❌ Sistema no inicializado');
        return;
    }
    
    const stats = app.tradingSystem.mlModel.getEvolutionStats();
    const statusElement = document.getElementById('modelStatusText');
    
    if (statusElement) {
        statusElement.innerHTML = `
            <span style="color: ${stats.isTrained ? '#00ff88' : '#ff4757'}">
                ${stats.isTrained ? 'ENTRENADO' : 'NO ENTRENADO'}
            </span> | 
            Accuracy: ${(stats.currentAccuracy * 100).toFixed(1)}% |
            Sesiones: ${stats.trainingSessions}
        `;
    }
    
    console.log('🔍 Estado del modelo:', stats);
}

// ✅ FUNCIÓN DE VERIFICACIÓN DE ALMACENAMIENTO
function checkStorageStatus() {
    if (!app || !app.tradingSystem || !app.tradingSystem.mlModel.persistence) {
        alert('❌ Persistencia no disponible');
        return;
    }
    
    const storageStatus = app.tradingSystem.mlModel.persistence.checkStorageStatus();
    const allModels = app.tradingSystem.mlModel.persistence.loadAllModels();
    
    alert(`📊 Estado Almacenamiento:\n
• Modelos guardados: ${Object.keys(allModels).length}
• Espacio usado: ${(storageStatus.used / 1024).toFixed(2)} KB
• Estado: ${storageStatus.status === 'healthy' ? '✅ SALUDABLE' : '⚠️ ADVERTENCIA'}
• Porcentaje usado: ${storageStatus.percentage.toFixed(1)}%`);
    
    console.log('📊 Estado almacenamiento:', storageStatus);
}
