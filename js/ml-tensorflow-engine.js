...
class ForexTensorFlowModel {
    ...
    async buildModel() {
        const model = tf.sequential({
            layers: [
                // Primera capa LSTM con dropout e inicializadores rápidos
                tf.layers.lstm({
                    units: 128,
                    ...
                    kernelInitializer: 'glorotUniform',      // ✅ OPTIMIZADO
                    recurrentInitializer: 'glorotUniform'    // ✅ OPTIMIZADO
                }),
                
                // Batch Normalization para estabilidad
                tf.layers.batchNormalization(),
                
                // Segunda capa LSTM con inicializadores rápidos
                tf.layers.lstm({
                    units: 64,
    ...
    // ✅ NUEVO: Calcular pesos de clase para balancear datos
    calculateClassWeights(yLabels) {
        const counts = { 0: 0, 1: 0, 2: 0 }; // BUY, SELL, HOLD
        ...
        console.log('⚖️ Pesos de clase calculados:', weights);
        return weights;
    }
    ...
    async trainOnRealData(historicalData, epochs = 30, validationSplit = 0.2) {
        ...
        try {
            ...
            const history = await this.model.fit(xTrain, yTrain, {
                ...
                classWeight: classWeights,  // ✅ AGREGADO: Balanceo de clases
                shuffle: true,
                callbacks: callbacks,
                verbose: 0
            });
            ...
            // ✅ GUARDAR MODELO COMPLETO EN FIREBASE CON LOGS DETALLADOS
            ...
            if (saveSuccess) {
                ...
                console.warn('⚠️ Hubo un problema guardando el modelo');
            }
    ...
    prepareTrainingData(enrichedData) {
        ...
        // Split train/validation PRIMERO
        ...
        // Normalizar SOLO con datos de training
        ...
        // Normalizar validación con scaler de training
...
calculateFutureMove(futureData) {
    ...
    if (this.instrument === 'XAUUSD') {
        // Oro: movimientos moderados (0.15% en vez de 0.4%)
        buyThreshold = 0.0015;   // ✅ CORREGIDO
        sellThreshold = 0.0015;
    } else if (this.instrument === 'EURGBP') {
        // EUR/GBP: par muy estable, se mueve MUY poco
        buyThreshold = 0.0003;   // ✅ AGREGADO (0.03%)
    ...
    // ✅ MEJORADO: Manejo de errores detallado y logs completos
    async saveModelToFirebase() {
        try {
            console.log(`💾 Iniciando guardado de ${this.instrument}...`);
            
            // 1. Guardar pesos en IndexedDB (navegador)
            ...
            // 3. Guardar último entrenamiento en historial
            if (this.trainingHistory.length > 0) {
                ...
                try {
                    ...
                    console.warn('⚠️ No se pudo guardar historial:', historyError.message);
