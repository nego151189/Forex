// trading-system.js - SISTEMA DE TRADING CON GESTIÓN DE RIESGO MEJORADA Y CORREGIDA
class AdvancedTradingSystem {
    constructor(instrument = 'USDZAR') {
        ...
        // ✅ CONFIGURACIÓN MEJORADA Y CORREGIDA DE RIESGO
        this.riskConfig = {
            // ✅ R/R MÍNIMO MÁS REALISTA - CORREGIDO
            minRiskRewardRatio: 1.2,    // 1.2:1 mínimo (R/R ALTO ES BUENO)
            maxRiskRewardRatio: 10.0,   // Permitir R/R altos (10:1 es EXCELENTE)
            
            // ✅ STOP LOSS MÁS AGRESIVO
            stopLossATRMultiplier: 1.2,
            stopLossPercent: 0.003,     // 0.3% en lugar de ~0.5%
            
            // ✅ TAKE PROFIT MÁS REALISTA
            takeProfitATRMultiplier: 2.0,
            takeProfitPercent: 0.008,   // 0.8% en lugar de ~1.2%
            
            // ✅ CONFIANZA MÍNIMA REQUERIDA
            minConfidence: 0.60,        // 60% mínimo
            highConfidence: 0.75        // 75% para señales fuertes
        };
    ...
    // ✅ GENERAR SEÑAL COMPLETA DE TRADING CON GESTIÓN DE RIESGO MEJORADA
    async generateTradingSignal(instrument, historicalData) {
        try {
            ...
            // 4. CALCULAR POSICIÓN EXACTA CON GESTIÓN DE RIESGO MEJORADA Y CORREGIDA
    ...
    // ✅ CALCULAR POSICIÓN EXACTA CON GESTIÓN DE RIESGO MEJORADA Y CORREGIDA
    calculateExactPosition(signal, data) {
        const currentPrice = data[data.length - 1].close;
        const atr = this.calculateATR(data.slice(-14));
        const volatility = this.calculateVolatility(data);
        
        // ✅ USAR GESTIÓN DE RIESGO MEJORADA Y CORREGIDA
        ...
        // ✅ CORREGIR LÓGICA DE VALIDACIÓN - R/R ALTO ES BUENO
        if (!riskManagement.isValid) {
            console.log(`⚠️ Señal filtrada - R/R: ${riskManagement.riskRewardRatio.toFixed(2)}:1, Confianza: ${(signal.confidence * 100).toFixed(1)}%`);
            return {
                action: 'HOLD',
        ...
        // ✅ SEÑAL APROBADA - MOSTRAR INFORMACIÓN CORRECTA
        ...
        return {
            ...
            confidence: signal.confidence, // ✅ Usar confianza real, no 95%
    ...
    // ✅ GESTIÓN DE RIESGO MEJORADA Y CORREGIDA
    calculateAdvancedRiskManagement(direction, currentPrice, atr, volatility, confidence) {
        ...
        if (direction === 'BUY') {
            ...
            takeProfitPrice = currentPrice + takeProfit;
        ...
        // ✅ CORREGIR VALIDACIÓN: R/R ALTO ES BUENO, BAJO ES MALO
        const isValid = riskRewardRatio >= this.riskConfig.minRiskRewardRatio && 
                       confidence >= this.riskConfig.minConfidence;
        
        console.log(`📊 Risk Management: R/R=${riskRewardRatio.toFixed(2)}:1, SL=${stopLossPrice.toFixed(4)}, TP=${takeProfitPrice.toFixed(4)}, Valid=${isValid}`);
    ...
    calculateOptimalEntry(currentPrice, direction, atr) {
        const buffer = atr * 0.1; // Buffer reducido para mejores entradas
        if (direction === 'BUY') {
    ...
    calculatePositionSize(entryPrice, stopLoss, confidence) {
        ...
        return Math.min(positionSize * confidence, 10); // Limitar tamaño máximo
    }
