...
class MLPersistenceSystem {
    ...
    // ✅ GUARDAR MODELO COMPLETO CON LOGS MEJORADOS
    async saveModel(modelId, mlModel) {
        try {
            console.log(`💾 [PERSISTENCE] Intentando guardar modelo: ${modelId}`);
            console.log(`💾 [PERSISTENCE] Estado - isTrained: ${mlModel.isTrained}, accuracy: ${mlModel.accuracy}`);
            // EN SU LUGRO, PERMITIR GUARDADO SIEMPRE:
            console.log(`💾 [PERSISTENCE] Guardando modelo sin importar accuracy: ${mlModel.accuracy}`);
            ...
            // ✅ === LOGS DE VERIFICACIÓN AGREGADOS ===
            console.log(`💾 [PERSISTENCE] Guardando en localStorage...`);
            localStorage.setItem(this.storageKey, JSON.stringify(existingData));

            this.models.set(modelId, modelData);

            // Verificar después de guardar
            const verifyData = this.loadAllModels();
            console.log(`✅ [PERSISTENCE] Modelo ${modelId} guardado EXITOSAMENTE`);
            console.log(`✅ [PERSISTENCE] Modelos en storage:`, Object.keys(verifyData));
            // === FIN LOGS DE VERIFICACIÓN ===
...
// REEMPLAZAR en ml-persistence.js la función saveTrainingSession:

async saveTrainingSession(modelId, sessionData) {
    try {
        console.log(`📝 [PERSISTENCE] Guardando sesión para: ${modelId}`, sessionData);
        
        const historyKey = `${this.trainingHistoryKey}-${modelId}`;
        
        // ✅ CORRECCIÓN: Manejar mejor el caso de datos corruptos
        let existingHistory = [];
        try {
        ...
        console.log(`📝 [PERSISTENCE] Historial existente: ${existingHistory.length} sesiones`);
        ...
        console.log(`📝 [PERSISTENCE] Guardando ${trimmedHistory.length} sesiones...`);
        localStorage.setItem(historyKey, JSON.stringify(trimmedHistory));

        // ✅ VERIFICACIÓN INMEDIATA MEJORADA
        setTimeout(() => {
            try {
                const verifyData = JSON.parse(localStorage.getItem(historyKey) || '[]');
                console.log(`✅ [PERSISTENCE] Verificación: ${verifyData.length} sesiones guardadas para ${modelId}`);
            } catch (verifyError) {
                console.error('❌ [PERSISTENCE] Error en verificación:', verifyError);
            }
        }, 100);
...
// ✅ FUNCIÓN loadTrainingHistory COMPLETA (verificar que existe así)
async loadTrainingHistory(modelId) {
    try {
        const historyKey = `${this.trainingHistoryKey}-${modelId}`;
        const historyData = localStorage.getItem(historyKey);
        
        // ✅ CORREGIR: Asegurar que siempre retorne un array
        if (!historyData) {
            console.log(`📊 [DEBUG] No hay historial para ${modelId}`);
            return [];
        }
...
class PersistentForexMLModel extends ForexMLModel {
    ...
    getEvolutionStats() {
        ...
        // ✅ NUEVA FUNCIÓN AGREGADA - Verificar existencia de modelo
    checkModelExists(modelId) {
        const allModels = this.loadAllModels();
        const exists = !!allModels[modelId];
        console.log(`🔍 [PERSISTENCE] Verificando modelo ${modelId}: ${exists ? 'EXISTE' : 'NO EXISTE'}`);
        return exists;
    }
